...
public interface UserRepository extends JpaRepository<User, Long> {
    @Query("select t from User t where t.name =:name")  // 这个Query不是数据库查询的sql
    User findByUserName(@Param("name") String name);//* User表操作接口 */
}

/**
 * 对于 stereotype下提供的 Repository标签有如下认识；
 *
